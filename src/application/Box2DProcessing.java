...
public class Box2DProcessing {
	...
	public Box2DProcessing(float sf) {
		transX = (float) Main.canvas.getWidth()/2;
		transY = (float) Main.canvas.getHeight()/2;
		scaleFactor = sf;
		yFlip = -1;
	...
	public Vec2 coordWorldToPixels(float worldX, float worldY) {
		float pixelX = Utils.map(worldX, 0f, 1f, transX, transX+scaleFactor);
		float pixelY = Utils.map(worldY, 0f, 1f, transY, transY+scaleFactor);
		if (yFlip == -1.0f) pixelY = Utils.map(pixelY,0f,(float) Main.canvas.getHeight(),
				(float) Main.canvas.getHeight(),0f);
	...
	public Vec2 coordPixelsToWorld(float pixelX, float pixelY) {
		float worldX = Utils.map(pixelX, transX, transX+scaleFactor, 0f, 1f);
		float worldY = pixelY;
		if (yFlip == -1.0f) worldY = Utils.map(pixelY,(float) Main.canvas.getHeight(),0f,0f,
				(float) Main.canvas.getHeight());
		worldY = Utils.map(worldY, transY, transY+scaleFactor, 0f, 1f);
